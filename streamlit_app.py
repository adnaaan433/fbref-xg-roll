...
if url:
    try:
        ...
        columns_to_keep = ['Date', 'Comp', 'Round', 'Squad', 'Opponent', 
                          ...
                          'Expected_npxG', 'Expected_xAG', 'Season']
        
        # Only keep columns that exist in the dataframe
        columns_to_keep = [col for col in columns_to_keep if col in match_logs_df.columns]
        match_logs_df = match_logs_df[columns_to_keep]
        ...
        key_cols = ['Date', 'Comp', 'Round', 'Squad', 'Opponent']
        tmp = match_logs_df.copy()
        tmp['_npxg_num'] = pd.to_numeric(tmp.get('Expected_npxG', pd.Series()), errors='coerce')
        tmp['_has_npxg'] = tmp['_npxg_num'].notna().astype(int)
        tmp = tmp.sort_values(key_cols + ['_has_npxg'], ascending=[True, True, True, True, True, False])
        tmp = tmp.drop_duplicates(subset=key_cols, keep='first')
        match_logs_df = tmp.drop(columns=['_npxg_num', '_has_npxg'])
        ...
        # Dynamic color picker labels based on chart type
        if chart_type == "npxG vs npGoals":
            label1 = "Non-Penalty Goals Color"
            label2 = "Non-Penalty xG Color"
            help1 = "Color for the npGoals line and overperforming areas"
            help2 = "Color for the npxG line and underperforming areas"
        elif chart_type == "xAG vs Assists":
            label1 = "Assists Color"
            label2 = "xAG Color"
            help1 = "Color for the Assists line and overperforming areas"
            help2 = "Color for the xAG line and underperforming areas"
        else:  # Combined
            label1 = "Goals+Assists Color"
            label2 = "xG+xAG Color"
            help1 = "Color for the npGoals+Assists line and overperforming areas"
            help2 = "Color for the npxG+xAG line and underperforming areas"
        
        with col1:
            actual_color = st.color_picker(label1, value="#00fff2", help=help1)
        
        with col2:
            expected_color = st.color_picker(label2, value="#e217f7", help=help2)
        
        # Create chart dataframe with rolling averages
        st.markdown("---")
        st.subheader("📊 Rolling Average Performance Chart")
        
        # Calculate rolling averages based on chart type
        chart_df = match_logs_df.copy()
        
        # Convert numeric columns to proper numeric types
        if 'Performance_Gls' in chart_df.columns:
            chart_df['Performance_Gls'] = pd.to_numeric(chart_df['Performance_Gls'], errors='coerce')
        if 'Performance_Ast' in chart_df.columns:
            chart_df['Performance_Ast'] = pd.to_numeric(chart_df['Performance_Ast'], errors='coerce')
        if 'Performance_PK' in chart_df.columns:
            chart_df['Performance_PK'] = pd.to_numeric(chart_df['Performance_PK'], errors='coerce')
        if 'Expected_npxG' in chart_df.columns:
            chart_df['Expected_npxG'] = pd.to_numeric(chart_df['Expected_npxG'], errors='coerce')
        if 'Expected_xAG' in chart_df.columns:
            chart_df['Expected_xAG'] = pd.to_numeric(chart_df['Expected_xAG'], errors='coerce')
        
        # Calculate metrics based on chart type
        if chart_type == "npxG vs npGoals":
            chart_df['Actual_Metric'] = chart_df['Performance_Gls'] - chart_df['Performance_PK']
            chart_df['Expected_Metric'] = chart_df['Expected_npxG']
            title_text = f'{player_name} | <npGoals> vs <npxG>'
            ylabel = '10 match rolling average'
            
        elif chart_type == "xAG vs Assists":
            chart_df['Actual_Metric'] = chart_df['Performance_Ast']
            chart_df['Expected_Metric'] = chart_df['Expected_xAG']
            title_text = f'{player_name} | <Assists> vs <xAG>'
            ylabel = '10 match rolling average'
            
        else:  # Combined
            chart_df['Actual_Metric'] = (chart_df['Performance_Gls'] - chart_df['Performance_PK']) + chart_df['Performance_Ast']
            chart_df['Expected_Metric'] = chart_df['Expected_npxG'] + chart_df['Expected_xAG']
            title_text = f'{player_name} | <npGoals+Assists> vs <npxG+xAG>'
            ylabel = '10 match rolling average'
        
        # Calculate rolling averages
        chart_df['Rolling_avg_Actual'] = chart_df['Actual_Metric'].rolling(window=10, min_periods=1).mean()
        chart_df['Rolling_avg_Expected'] = chart_df['Expected_Metric'].rolling(window=10, min_periods=1).mean()
        chart_df['Match_Number'] = range(1, len(chart_df) + 1)
        
        # Check if we have valid data
        if chart_df['Rolling_avg_Actual'].isna().all() or chart_df['Rolling_avg_Expected'].isna().all():
            st.warning(f"No data available for {chart_type}. The required columns may be missing from the match logs.")
        else:
            # Create the line chart using matplotlib
            fig, ax = plt.subplots(figsize=(19.8, 10.8))
            
            # Plot the lines
            ax.plot(chart_df['Match_Number'], chart_df['Rolling_avg_Actual'], 
                    color=actual_color, linewidth=2)
            ax.plot(chart_df['Match_Number'], chart_df['Rolling_avg_Expected'], 
                    color=expected_color, linewidth=2)
            
            # If multiple seasons selected, mark season boundaries and annotate season names
            if 'Season' in chart_df.columns and chart_df['Season'].nunique() > 1:
                season_start_mask = chart_df['Season'].ne(chart_df['Season'].shift())
                season_starts = chart_df.loc[season_start_mask, ['Match_Number', 'Season']]
                y_max = float(max(chart_df['Rolling_avg_Actual'].max(), chart_df['Rolling_avg_Expected'].max()))
                y_min = float(min(chart_df['Rolling_avg_Actual'].min(), chart_df['Rolling_avg_Expected'].min()))
                y_pad = (y_max - y_min) * 0.08 if y_max > y_min else 0.1
                for idx, row in season_starts.iterrows():
                    x_pos = row['Match_Number']
                    season_label = str(row['Season'])
            ...
            # Fill areas between lines
            ax.fill_between(chart_df['Match_Number'], 
                           chart_df['Rolling_avg_Actual'], 
                           chart_df['Rolling_avg_Expected'],
                           where=(chart_df['Rolling_avg_Actual'] >= chart_df['Rolling_avg_Expected']),
                           color=actual_color, alpha=0.3, interpolate=True)
            
            ax.fill_between(chart_df['Match_Number'], 
                           chart_df['Rolling_avg_Actual'], 
                           chart_df['Rolling_avg_Expected'],
                           where=(chart_df['Rolling_avg_Actual'] < chart_df['Rolling_avg_Expected']),
                           color=expected_color, alpha=0.3, interpolate=True)
            ...
            ax.tick_params(axis='y', colors='white')
            ax.get_xaxis().set_visible(False)
            ax.set_ylabel(ylabel, fontsize=15, fontweight='bold', color='white', labelpad=15)
            
            # Create custom title with colored text using highlight_text
            fig_text(x=0.05, y=1, s=title_text, 
                    highlight_textprops=[
                        {"color": actual_color, "fontweight": "bold"},
                        {"color": expected_color, "fontweight": "bold"}
                    ],
                    fontsize=30, fontweight='bold', ha='left', va='center',
                    color='white', transform=fig.transFigure)

            fig.text(0.05, 0.95, "Data: fbref  |  Made by: Mohammad Adnan (@adnaaan433) For the Real Deal Podcast (@Realdealpodz)", 
                    color='white', fontsize=15, ha='left', va='center')
            
            # Add logos to top right corner
            try:
                realdeal_logo = plt.imread("RealDeal_Logo.png")
                imagebox1 = OffsetImage(realdeal_logo, zoom=0.12)
                ab1 = AnnotationBbox(imagebox1, (0.875, 1.05), frameon=False, 
                                   xycoords='figure fraction', box_alignment=(0, 1))
                ax.add_artist(ab1)
            except Exception as e:
            ...
            st.download_button(
                label="📥 Download Chart as PNG",
                data=buf.getvalue(),
                file_name=f"{player_name.replace(' ', '_')}_{chart_type.replace(' ', '_')}_chart.png",
                mime="image/png",
            )
